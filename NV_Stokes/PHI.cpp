...
void PHI::PHI_Initializer()
{
	...
	TempatureField.resize(MeshSize_X + 2, vector<double>(MeshSize_Y + 2));
}
...
void PHI::T_BCs(float T_L, float T_R)
{
	/*Left BC.*/
	for (int j = 0; j <= MeshSize_Y + 1; ++j)
	{
		TempatureField[0][j] = T_L;
	}
	/*Right BC.*/
	for (int j = 0; j <= MeshSize_Y + 1; ++j)
	{
		TempatureField[MeshSize_X + 1][j] = T_R;
	}
	/*Remember to Impose the B.Cs of the Upper and Bottom Walls*/
}
