...
/*Functions Declaration | _R_T_ */
void _R_T_Initializer(vector<vector<double>>& _R_T_, float MeshSize_X, float MeshSize_Y);
vector<vector<double>> _R_T_OPS(Cell mainMesh,Area Areas, u_Staggered_X u_Staggered_X_, v_Staggered_Y v_Staggered_Y_, double lambda, double density, double C_p , float MeshSize_X, float MeshSize_Y, vector<vector<double>> _R_T_, vector<vector<double>> T_Field);
...
vector<vector<double>> _T_OPS(vector<vector<double>> T_n, vector<vector<double>> T_new,vector<vector<double>>& _R_T_, vector<vector<double>>& _R_T_Prev, double Delta_t, double roh, double C_p, float MeshSize_X, float MeshSize_Y, const string& spatialOperator);


/*CFL Condition | Δt */
double CFL(u_Staggered_X u_Staggered_X_, v_Staggered_Y v_Staggered_Y_, double MeshSize_X, double MeshSize_Y, double lambda, double C_p, double roh, double Viscosity);
...
int main()
{
    double L = 1.0, H = 1.0, Nx = 10, My = 10, mu = 1, lambda= 1, C_p = 0.71, T_L = 1, T_R = 0;
    /*Define Re*/
    double Ra = 100, Beta = 1.0 / (0.5 + 273.15), g = Ra / (C_p * Beta);
    string Scheme = "CDS";
    double roh = 1, Conv_Crit = 1E-6, t = 0.0, timeElapsed = 0.0, U = 1.0;
    int itrMax = 30000, LoopNo = 1;
    vector<vector<double>> _R_U_, _R_V_, U_Pred, V_Pred, _R_U_Prev, _R_V_Prev, FinalPressureField, _R_T_, _R_T_Prev, Temp_Field_New;
    double tolerance = 1E-3; /*Temporal Loop*/
    bool isSteadyState = false;
    double Simulation_time = 40; /* a Rule of thumb: The fluid should pass through the domain 10 times to reach S.S. */
    ...
    double timeStep = CFL(u_Staggered_X_, v_Staggered_Y_, Nx, My, lambda,C_p, roh, mu);
    ...
    Temp_Field_New.resize(Nx + 2, vector<double>(My + 2));
    Export Export_Obj;
    /* *-*-*-*-*-*-*-**-*-*- Temporal Loop *-*-*-*-*-*-*-**-*-*- */
    while (timeElapsed < Simulation_time) {
        ...
        vector<vector<double>> T_Field = PHI.get_T();
        /*R Operations | Predictor Velocity*/
        _R_T_Initializer(_R_T_,Nx, My);
        _R_T_ = _R_T_OPS(Cell_Struct, Area_Struct, u_Staggered_X_, v_Staggered_Y_, lambda, roh, C_p, Nx, My, _R_T_, T_Field);
        ...
        if (LoopNo == 2) { _R_U_Prev = _R_U_;  _R_T_Prev = _R_T_; }
        ...
        if (LoopNo == 2) { _R_V_Prev = _R_V_; _R_T_Prev = _R_T_;}
        V_Pred = V_Predictor(_R_V_, _R_V_Prev, V_Pred, v_Staggered_Y_, timeStep, roh, Nx, My, "AdamBashforth");

        Temp_Field_New = _T_OPS(T_Field, Temp_Field_New, _R_T_, _R_T_Prev, timeStep, roh, C_p, Nx, My, "AdamBashforth");
        ...
        /* CFL Condition */
        timeStep = CFL(uCurrent, vCurrent, Nx, My, lambda, C_p, roh, mu);
        //timeStep = 0.0005;
    ...
    /*Writer for MATLAB*/
    CSVMATLAB uWriter("uCurrent_MATLAB_Re1000_64_UDS.csv");
    CSVMATLAB vWriter("vCurrent_MATLAB_Re1000_64_UDS.csv");
    CSVMATLAB PressureWriter("PressureField_Re1000_64_UDS.csv");
    CSVMATLAB v_xWritter("v_x_Re1000_64_UDS.csv");
    CSVMATLAB u_yWritter("u_y_Re1000_64_UDS.csv");
...
void _R_T_Initializer(vector<vector<double>>& _R_T_, float MeshSize_X, float MeshSize_Y)
{
    _R_T_.resize(MeshSize_X + 2, vector<double>(MeshSize_Y + 2));
}
...
vector<vector<double>> _R_T_OPS(Cell mainMesh, Area Areas, u_Staggered_X u_Staggered_X_, v_Staggered_Y v_Staggered_Y_, double lambda, double density, double C_p, float MeshSize_X, float MeshSize_Y, vector<vector<double>> _R_T_, vector<vector<double>> T_Field)
{
    ...
    for (int i = 1; i <= MeshSize_X; ++i) {
        for (int j = 1; j <= MeshSize_Y; ++j) {
            double conv_term = -density * C_p * (u_Staggered_X_.u_P[i][j] * Areas.East[i] *  T_Field[i + 1][j]  
                - u_Staggered_X_.u_P[i - 1][j] * Areas.West[i] * T_Field[i - 1][j] 
                + v_Staggered_Y_.v_P[i][j] * Areas.North[j] * T_Field[i][j + 1]  
                - v_Staggered_Y_.v_P[i][j - 1] * Areas.South[j] * T_Field[i][j - 1]);
            double diff_term_x = lambda * ((T_Field[i + 1][j] - T_Field[i][j]) * Areas.East[i] / mainMesh.dPE[i] + (T_Field[i][j] - T_Field[i - 1][j]) * Areas.West[i] / mainMesh.dPW[i]);
            double diff_term_y = lambda * ((T_Field[i][j + 1] - T_Field[i][j]) * Areas.North[j] / mainMesh.dPN[j] + (T_Field[i][j] - T_Field[i][j - 1]) * Areas.South[i] / mainMesh.dPS[j]);
            _R_T_[i][j] = (conv_term + diff_term_x + diff_term_y) / mainMesh.Volume[i][j];
        }
    }
...
vector<vector<double>> _T_OPS(vector<vector<double>> T_n, vector<vector<double>> T_new, vector<vector<double>>& _R_T_, vector<vector<double>>& _R_T_Prev, double Delta_t, double roh, double C_p, float MeshSize_X, float MeshSize_Y, const string& spatialOperator)
{
    for (int i = 1; i <= MeshSize_X; ++i)
        for (int j = 1; j < MeshSize_Y; ++j)
        {
            {
                if (spatialOperator == "AdamBashforth") {
                    T_new[i][j] = T_n[i][j] + (Delta_t / (roh * C_p) ) * (1.5 * _R_T_[i][j] - 0.5 * _R_T_Prev[i][j]);
                }
                else if (spatialOperator == "ForwardEuler") {
                    T_new[i][j] = T_n[i][j] + (Delta_t / (roh * C_p) ) * (1.0 * _R_T_[i][j] - 0.0 * _R_T_Prev[i][j]);
                }
...
double CFL(u_Staggered_X u_Staggered_X_, v_Staggered_Y v_Staggered_Y_, double MeshSize_X, double MeshSize_Y, double lambda, double C_p, double roh, double Viscosity)
{
    ...
    for (int i = 1; i <= MeshSize_X; ++i) {
        for (int j = 1; j <= MeshSize_Y; ++j) {
            ...
            CFL_Thermal[i][j] = 0.2 * dX * dX / (lambda / (roh * C_p));
        }
    }
    ...
    for (int i = 1; i <= MeshSize_X; ++i) {
        for (int j = 1; j <= MeshSize_Y; ++j) {
            double delta_t = std::min({CFL_Cond[i][j], CFL_Conv[i][j], CFL_Thermal[i][j] });
